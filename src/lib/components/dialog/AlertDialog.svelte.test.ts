...
const test = base.extend<AlertDialogFixture>({
...
describe("AlertDialog.svelte", () => {
    test("dialog opens", async ({ fixture: { alertDialog, onOpen, onCancel, onConfirm } }) => {
        alertDialog.open();
    ...
    test("confirm action works", async ({
        fixture: { alertDialog, user, onOpen, onCancel, onConfirm },
    }) => {
        alertDialog.open();
        expect(await screen.findByRole("alertdialog")).toBeVisible();
